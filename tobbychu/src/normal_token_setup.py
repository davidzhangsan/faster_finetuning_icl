# -*- coding: utf-8 -*-
"""Normal_token_setup.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1exWgO265ThQK9HubpkDD0H-bp3lhpUR0
...
num_heads = 3 #@param {type:"integer"}
...
config.training_steps_gd = 40000
...
#### Optimizer configs
config.adam = True  # Using SGD with momentum instead
...
def display_learning(train, test=None, gt=None, inter=None, title="train",
                     ...
                     color_axis=True,
                     height=3.5, width=5, ax1=None, ax2=None):  # Modified default size

    """Update learning curve image with larger dimensions and thinner lines."""
    ...
    if plot_num == 1:
        ...
        fig.set_size_inches(width, height)  # Increased figure size
    ...
    if test is not None and not second_axis:
        ...
        if len(test_list) > 1:
            if single_seeds:
                for s in test_list:
                    ax1.plot(x_range, s, color=colors(0.1+color_add), alpha=0.2, linewidth=1)  # Thinner line
            else:
                ax1.fill_between(x_range, test-test_std, test+test_std, alpha=0.2, facecolor=colors(0.1+color_add))
        ax1.plot(x_range[:len(test)], test, color=colors(0.1+color_add), label=title2, linewidth=1.5)  # Thinner line

    if gt is not None:
        if not second_axis:
            x_range = np.arange(0, num_iter_os, int(num_iter_os/len(gt)))
            ax1.plot(x_range[:len(gt)], gt, color=colors(0.2+color_add), label=title3, linewidth=1.5)  # Thinner line
            if len(gt_list) > 1:
                if single_seeds:
                    for s in gt_list:
                        ax1.plot(x_range, s, color=colors(0.2+color_add), alpha=0.2, linewidth=1, zorder=0)  # Thinner line
                else:
                    ax1.fill_between(x_range, gt-gt_std, gt+gt_std, alpha=0.2, facecolor=colors(0.2+color_add))
        else:
            x_range = np.arange(0, num_iter_os, int(num_iter_os/len(gt)))
            ax1.plot(x_range, gt, color=colors(0.6+color_add), label=title3, linewidth=1.5)  # Thinner line
            if len(gt_list) > 1:
                if single_seeds:
                    for s in gt_list:
                        ax1.plot(x_range, s, color=colors(0.6+color_add), alpha=0.3, linewidth=1, zorder=0)  # Thinner line
                else:
                    ax1.fill_between(x_range, gt-gt_std, gt+gt_std, alpha=0.2, facecolor=colors(0.6+color_add))

    if test is not None and second_axis:
        x_range = np.arange(0, num_iter_os, int(num_iter_os/len(test)))
        ax1.plot(x_range[:len(test[:-rw])], test[:-rw], color=colors(0.5+color_add), label=title2, linewidth=1.5)  # Thinner line
        if len(test_list) > 1:
            if single_seeds:
                for s in test_list:
                    ax1.plot(x_range, s, color=colors(0.5+color_add), linewidth=1, alpha=0.3, zorder=0)  # Thinner line
            else:
    ...
    if inter is not None and not second_axis:
        ...
        ax1.plot(x_range, inter, color=colors(0.4+color_add), label=title4, linewidth=1.5, zorder=10)  # Thinner line
        if len(inter_list) > 1:
            if single_seeds:
                for s in inter_list:
                    ax1.plot(x_range, s, color=colors(0.4+color_add), alpha=0.3, linewidth=1, zorder=0)  # Thinner line
            else:
    ...
    if second_axis:
        ...
        ax2.plot(x_range, train, color=colors(0.4+color_add), label=title1, linewidth=1.5)  # Thinner line
        ax2.plot(x_range, np.ones_like(train), "--", color="gray", linewidth=0.5)  # Thinner dashed line
        if len(train_list) > 1:
            if single_seeds:
                for s in train_list:
                    ax1.plot(x_range, s, line, color=colors(0.4+color_add), alpha=0.3, linewidth=1, zorder=0)  # Thinner line
            else:
        ...
        if line != "-":
            ax1.scatter(x_range, train, s=[50 for _ in x_range],  # Reduced marker size
                       marker="+", color=colors(0.3+color_add), alpha=1, label=title1, zorder=3, linewidths=1.5)  # Thinner lines
        else:
            ax1.plot(x_range, train, line, color='blue', label=title1, linewidth=2, zorder=11)  # Thinner line
        if len(train_list) > 1:
            if single_seeds:
                for s in train_list:
                    ax1.plot(x_range, s, line, color=colors(0.3+color_add), alpha=0.3, linewidth=1, zorder=0)  # Thinner line
            else:
...
losses_gd_list = [0.01]*len(loss_trans_list[0])
...
display_learning(loss_trans_list, [losses_gd_list], y_lim_u=0.8, y_lim_l=0.0,
                 loc_first='upper right', color_add=0.0, title2='GD++ 5 steps', title1="TF 5 layers",
                 rw=1, title="train.pdf", allow_download=False, single_seeds=True,
                 num_iter_os=len(losses_gd_list)*500)

#@title Visualize sensitivity
display_learning(other_list_half, own_list_half, own_one_list_half, y_lim_u=4.5, y_lim_l=0.0,
